...
#define NANODBC_SQLWCHAR SQLWCHAR
#else
...
inline std::size_t size(NANODBC_SQLWCHAR const (&array)[N]) noexcept
{
    auto const n = std::char_traits<NANODBC_SQLWCHAR>::length(array);
    NANODBC_ASSERT(n < N);
    return n < N ? n : N - 1;
}
...
recent_error(SQLHANDLE handle, SQLSMALLINT handle_type, long& native, std::string& state)
{
    ...
    std::string rvalue;
    std::vector<NANODBC_SQLWCHAR> sql_message(SQL_MAX_MESSAGE_LENGTH);
    sql_message[0] = '\0';

    SQLINTEGER i = 1;
    SQLINTEGER native_error = 0;
    SQLSMALLINT total_bytes = 0;
    NANODBC_SQLWCHAR sql_state[6] = {0};
    RETCODE rc;
...
public:
        ...
        void* event_handle = nullptr)
    {
        ...
        NANODBC_CALL_RC(
            ...
            rc,
            dbc_,
            (NANODBC_SQLWCHAR*)dsn.c_str(),
            SQL_NTS,
            !user.empty() ? (NANODBC_SQLWCHAR*)user.c_str() : 0,
            SQL_NTS,
            !pass.empty() ? (NANODBC_SQLWCHAR*)pass.c_str() : 0,
            SQL_NTS);
    ...
    connect(const string& connection_string, long timeout, void* event_handle = nullptr)
    {
        ...
        NANODBC_CALL_RC(
            ...
            dbc_,
            0,
            (NANODBC_SQLWCHAR*)connection_string.c_str(),
            SQL_NTS,
            nullptr,
            0,
            nullptr,
    ...
    {
        NANODBC_SQLWCHAR name[SQL_MAX_OPTION_STRING_LENGTH] = {0};
        SQLINTEGER length(0);
        RETCODE rc;
        NANODBC_CALL_RC(
            ...
            name,
            sizeof(name) / sizeof(NANODBC_SQLWCHAR),
            &length);
        if (!success(rc))
            NANODBC_THROW_DATABASE_ERROR(dbc_, SQL_HANDLE_DBC);
...
string connection::connection_impl::get_info_impl<string>(short info_type) const
{
    ...
    NANODBC_CALL_RC(
        ...
        info_type,
        value,
        sizeof(value) / sizeof(NANODBC_SQLWCHAR),
        &length);
    if (!success(rc))
        NANODBC_THROW_DATABASE_ERROR(dbc_, SQL_HANDLE_DBC);
...
public:
        ...
        void* event_handle = nullptr)
    {
        ...
        NANODBC_CALL_RC(
            NANODBC_FUNC(SQLExecDirect), rc, stmt_, (NANODBC_SQLWCHAR*)query.c_str(), SQL_NTS);
        ...
        statement& statement)
    {
        ...
        NANODBC_CALL_RC(
            ...
            rc,
            stmt_,
            (NANODBC_SQLWCHAR*)(catalog.empty() ? nullptr : catalog.c_str()),
            (catalog.empty() ? 0 : SQL_NTS),
            (NANODBC_SQLWCHAR*)(schema.empty() ? nullptr : schema.c_str()),
            (schema.empty() ? 0 : SQL_NTS),
            (NANODBC_SQLWCHAR*)procedure.c_str(),
            SQL_NTS,
            (NANODBC_SQLWCHAR*)(column.empty() ? nullptr : column.c_str()),
...
public:
    ...
    string column_datatype_name(short column) const
    {
        ...
        NANODBC_SQLWCHAR type_name[256] = {0};
        ...
        NANODBC_ASSERT(len % sizeof(NANODBC_SQLWCHAR) == 0);
        len = len / sizeof(NANODBC_SQLWCHAR);
...
private:
    ...
    void auto_bind()
    {
        ...
        RETCODE rc;
        NANODBC_SQLWCHAR column_name[1024];
        ...
        for (SQLSMALLINT i = 0; i < n_columns; ++i)
        {
            NANODBC_CALL_RC(
                ...
                stmt_.native_statement_handle(),
                i + 1,
                (NANODBC_SQLWCHAR*)column_name,
                sizeof(column_name) / sizeof(NANODBC_SQLWCHAR),
                &len,
                &sqltype,
                &sqlsize,
                &scale,
...
{
    NANODBC_SQLWCHAR name[1024] = {0};
    NANODBC_SQLWCHAR driver[1024] = {0};
    ...
    do
    {
        NANODBC_CALL_RC(
            ...
            direction,                                // Direction
            name,                                     // DSN Name
            sizeof(name) / sizeof(NANODBC_SQLWCHAR),   // Size of Name Buffer
            &name_len_ret,                            // Written DSN Name length
            driver,                                   // Driver Name
            sizeof(driver) / sizeof(NANODBC_SQLWCHAR), // Size of Driver Buffer
        ...
        if (rc == SQL_SUCCESS)
        {
            ...
            //static_assert(
            //    sizeof(NANODBC_SQLWCHAR) == sizeof(char_type),
            //    "incompatible SQLCHAR and string::value_type");

            datasource dsn;
            dsn.name = string(&name[0], &name[std::char_traits<NANODBC_SQLWCHAR>::length(name)]);
            dsn.driver =
                string(&driver[0], &driver[std::char_traits<NANODBC_SQLWCHAR>::length(driver)]);
...
{
    NANODBC_SQLWCHAR descr[1024] = {0};
    NANODBC_SQLWCHAR attrs[1024] = {0};
    ...
    do
    {
        NANODBC_CALL_RC(
            ...
            sizeof(descr) / sizeof(NANODBC_SQLWCHAR), // BufferLength1
            &descr_len_ret,                          // DescriptionLengthPtr
            attrs,                                   // DriverAttributes
            sizeof(attrs) / sizeof(NANODBC_SQLWCHAR), // BufferLength2
        ...
        if (rc == SQL_SUCCESS)
        {
            ...
            driver drv;
            drv.name = string(&descr[0], &descr[std::char_traits<NANODBC_SQLWCHAR>::length(descr)]);
    ...
    const string& catalog)
{
    ...
    NANODBC_CALL_RC(
        ...
        (NANODBC_SQLWCHAR*)(schema.empty() ? nullptr : schema.c_str()),
        (schema.empty() ? 0 : SQL_NTS),
        (NANODBC_SQLWCHAR*)(table.empty() ? nullptr : table.c_str()),
        (table.empty() ? 0 : SQL_NTS),
        (NANODBC_SQLWCHAR*)(type.empty() ? nullptr : type.c_str()),
...
catalog::find_procedures(const string& procedure, const string& schema, const string& catalog)
{
    ...
    NANODBC_CALL_RC(
        ...
        (NANODBC_SQLWCHAR*)(schema.empty() ? nullptr : schema.c_str()),
        (schema.empty() ? 0 : SQL_NTS),
        (NANODBC_SQLWCHAR*)(procedure.empty() ? nullptr : procedure.c_str()),
...
std::list<string> catalog::list_catalogs()
{
    ...
    NANODBC_CALL_RC(
        ...
        rc,
        stmt.native_statement_handle(),
        (NANODBC_SQLWCHAR*)SQL_ALL_CATALOGS,
        1,
        (NANODBC_SQLWCHAR*)NANODBC_TEXT(""),
        0,
        (NANODBC_SQLWCHAR*)NANODBC_TEXT(""),
        0,
        (NANODBC_SQLWCHAR*)NANODBC_TEXT(""),
...
std::list<string> catalog::list_schemas()
{
    ...
    NANODBC_CALL_RC(
        ...
        (NANODBC_SQLWCHAR*)NANODBC_TEXT(""),
        0,
        (NANODBC_SQLWCHAR*)SQL_ALL_SCHEMAS,
        1,
        (NANODBC_SQLWCHAR*)NANODBC_TEXT(""),
        0,
        (NANODBC_SQLWCHAR*)NANODBC_TEXT(""),
